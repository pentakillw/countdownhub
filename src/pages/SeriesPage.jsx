...
import { Search, Filter, SlidersHorizontal, ChevronLeft, ChevronRight, Tv, Sparkles } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        setError("No se pudieron cargar las series. Intenta de nuevo más tarde.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div>
      ...
      {!loading && !error && !searchTerm && featuredEvents.length > 0 && (
        <div className="mb-12 relative group">
             <div className="flex items-center gap-2 mb-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
                <Tv className="text-deco-verde-1 animate-pulse" />
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-3xl md:text-4xl font-bold text-text-default flex items-center gap-2">
          <Sparkles className="text-deco-verde-1" />
          Nuevas Temporadas
        </h1>
        <button
      ...
      <div className="relative mb-6">
        <input
          type="text"
          placeholder="Buscar series... (Ej: Stranger Things)"
      ...
      {!loading && !error && filteredEvents.length === 0 && (
         <div className="text-center py-20 bg-white dark:bg-bg-muted rounded-lg shadow-md">
          ...
          <p className="text-lg text-text-subtle">No hay series con estos filtros.</p>
          <button
